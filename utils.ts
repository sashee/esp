...
export const processLogs = (contents: string) => {
	...
	const interestingIndices = numbers.flatMap((e, i) => [40, 13].includes(e) ? [{index: i, e}] : []);
	const blocks = interestingIndices.flatMap((elem, i, l) => {
		if (elem.e === 40 && l[i + 1]?.e === 13) {
			const bytes = new Uint8Array(numbers.slice(elem.index, l[i + 1].index));
			return [{bytes, crcOk: checkCrc(bytes)}];
		}else {
			return [];
		}
	});

	const crcGroups = Object.groupBy(blocks, ({crcOk}) => crcOk);
	const invalidCrcGroups = crcGroups[false];

	const messages: string[] = crcGroups[true].map(({bytes}) => new TextDecoder().decode(bytes.subarray(1, -2)));

	console.log(`Total number of blocks: ${messages.length}`);

	const parsedMessages = messages.map((message) => ({message, parsed: commands.reduce((memo, parser) => {
		if (memo) {
			return memo;
		}
	...
	}, undefined as undefined | {command: string, values: ReturnType<typeof commands[number]["parse"]>})}));
	const unparseableMessages = parsedMessages.filter(({parsed}) => parsed === undefined);
	const validMessages = parsedMessages.filter(({parsed}) => parsed !== undefined);
