...
Saved runs are JSON files discovered from the current directory. Filenames are the user-visible titles. The file does not store outbound events because they are regenerated by replay.
...
- `event_type`: `create_sync`, `create_async`, `return_sync`, `resolve_async`, `abort_async`, or `cancel_async`

The file does not store runtime numeric ids. Instead:

- request events define a symbolic `id`
- response/cancel/abort events refer to an existing symbolic `target`
...
- symbolic `id`
- the expected operation payload
...
- `target` must refer to an earlier symbolic operation id
...
4. for each `outbound` item, match it against the currently available derived outbound request
5. for each `inbound` item, build the concrete inbound runtime event and push it into the wrapper
6. collect the newly emitted outbound events
7. continue until the list ends or replay becomes invalid
...
Saved `outbound` items are persisted in the JSON document, but they should normally annotate their matched derived outbound events rather than appear as separate visible rows in the main timeline.
...
- symbolic id and target resolution
