    ...
    AR: Send + 'static,
{
    pub fn create_sync(&self, op: S) -> SR {
        let mut state = self.shared.lock();
        assert!(!state.shutdown, "driver shutdown during sync operation");
        ...
        loop {
            if state.shutdown {
                panic!("driver shutdown during sync operation");
            }
    ...
    pub fn create_async(&self, op: A) -> AsyncHandle<S, A, SR, AR> {
        let mut state = self.shared.lock();
        assert!(!state.shutdown, "driver shutdown during async operation");
