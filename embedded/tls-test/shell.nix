let
  ...
  build = pkgs.writeShellScriptBin "build" ''
    ...
    app_dir="''${ESP_TLS_TEST_APP_DIR-}"
    artifact_dir="''${APP_ARTIFACT_DIR-}"

    if [ -z "$app_dir" ] || [ -z "$artifact_dir" ]; then
      printf '%s\n' 'build must be run from inside the tls-test nix-shell' >&2
      exit 1
    fi
  ...
  flash = pkgs.writeShellScriptBin "flash" ''
    ...
    if [ -z "$app_dir" ] || [ -z "$artifact_dir" ]; then
      printf '%s\n' 'flash must be run from inside the tls-test nix-shell' >&2
      exit 1
    fi
  ...
  run = pkgs.writeShellScriptBin "run" ''
    ...
    if [ -z "$app_dir" ] || [ -z "$artifact_dir" ]; then
      printf '%s\n' 'run must be run from inside the tls-test nix-shell' >&2
      exit 1
    fi
    ...
    espflash flash --monitor "$artifact_path" "$@"
  '';
  ...
  watch = pkgs.writeShellScriptBin "watch" ''
    ...
    app_dir="''${ESP_TLS_TEST_APP_DIR-}"
    repo_root="''${ESP_TLS_TEST_REPO_ROOT-}"

    if [ -z "$app_dir" ] || [ -z "$repo_root" ]; then
      printf '%s\n' 'watch must be run from inside the tls-test nix-shell' >&2
      exit 1
    fi
...
pkgs.mkShell {
  ...
  shellHook = ''
    ...
    export RUSTC="${shared.rustToolchain}/bin/rustc"
    export RUSTDOC="${shared.rustToolchain}/bin/rustdoc"
    export CC_riscv32imac_esp_espidf="riscv32-esp-elf-gcc"
    export AR_riscv32imac_esp_espidf="riscv32-esp-elf-ar"
    export CXX_riscv32imac_esp_espidf="riscv32-esp-elf-g++"

    repo_root=$(git rev-parse --show-toplevel 2>/dev/null) || {
      printf '%s\n' 'tls-test shell must be entered from inside a git worktree' >&2
      exit 1
    }

    export ESP_TLS_TEST_REPO_ROOT="$repo_root"
    export ESP_TLS_TEST_APP_DIR="$repo_root/${shared.appDir}"
    export CARGO_TARGET_DIR="$ESP_TLS_TEST_APP_DIR/target"
    export APP_ARTIFACT_DIR="$ESP_TLS_TEST_APP_DIR/.artifacts"

    mkdir -p "$CARGO_TARGET_DIR" "$APP_ARTIFACT_DIR"

    if [ -t 1 ]; then
      printf '%s\n' "tls-test nix-shell rooted at: $ESP_TLS_TEST_REPO_ROOT"
